...
// Hard-coded rather than read from the CSS var so the off-screen template never depends on
// client-side style resolution when html2canvas clones it.
const PRIMARY = "#2563eb";

/**
 * Off-screen print layout captured by html2canvas for PDF export — plain HTML/CSS at a fixed
 * print width, with no app chrome, so the rasterized page reads cleanly at any zoom.
 */
export const InvoicePrintTemplate = forwardRef<HTMLDivElement, { invoice: CustomerInvoiceDetail }>(
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20, fontSize: 13 }}>
          ...
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontWeight: 700 }}>{INVOICE_STATUS_META[invoice.status].label}</div>
          </div>
        </div>
            ...
            {invoice.lines.map((line) => (
              <tr key={line.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                ...
                <td style={cellStyle("center")}>
                  {formatQuantity(line.quantity)} {formatUnit(line.unit_name)}
                </td>
                <td style={cellStyle("center")}>{money(line.unit_price)}</td>
                <td style={cellStyle("left")}>{money(line.subtotal)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {invoice.payments.length > 0 && (
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>الدفعات</div>
              ...
              <tbody>
                {invoice.payments.map((payment) => (
                  <tr key={payment.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                    ...
                    <td style={cellStyle("left")}>{money(payment.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ marginTop: 24, marginRight: "auto", width: 280 }}>
          <SummaryRow label="الإجمالي" value={money(invoice.total_amount)} />
          <SummaryRow label="المدفوع" value={money(invoice.paid_amount)} />
          {Number(invoice.returned_amount) > 0 && (
            <SummaryRow label="المرتجع" value={money(invoice.returned_amount)} />
          )}
          <SummaryRow label="المتبقي" value={money(invoice.balance_due)} strong />
        </div>
