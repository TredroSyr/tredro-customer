...
export function CartReviewDrawer({
  ...
  const total = cart.reduce((sum, i) => sum + i.quantity * Number(i.price ?? 0), 0);
  ...
  return (
    <Drawer
      ...
      onSnapPointChange={(snapPoint) => setExpanded(snapPoint === SNAP_POINTS[1])}
    >
      <DrawerContent>
        <DrawerHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 pt-2">
          <div className="flex items-center justify-between rounded-xl border border-border p-3">
            <span className="flex items-center gap-2 text-sm font-bold">
              <IconRenderer name="cart_outlined" className="size-4 text-primary" />
              {itemCount} منتج
            </span>
          ...
          {expanded && (
            <div className="animate-in fade-in-0 slide-in-from-bottom-2 space-y-4 duration-300">
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {item.price ? formatCurrency(item.price) : "السعر غير متاح"} × {item.quantity}
                      </p>
                    </div>
              ...
              <p className="text-[10px] text-muted-foreground">
                هذا طلب أولي وليس تأكيداً نهائياً. سيتواصل معك المندوب لتأكيد الأسعار والكميات.
              </p>
            </div>
          )}
        </div>
